...
namespace raptor {
    ...
    // Class that preallocates a buffer and appends strings to is. Reserve an additional byte for the null termination when needed.
    struct StringBuffer {
        ...
        char* append_use( const StringView& text );     // Append and returns a pointer to start. Used for strings mostly.
