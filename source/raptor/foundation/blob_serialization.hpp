...
namespace raptor {
    ...
    struct BlobSerializer {
        ...
        template <typename T>
        T*      write_and_serialize( Allocator* allocator, u32 serializer_version, sizet size, T* root_data );
        ...
        // Size is used to check whether reading is happening outside of the chunk.
        // Allocator is userd to allocate memory if needed (for example when reading an array).
        template <typename T>
        ...
        char*   allocate_static( sizet size ); // Just allocat size bytes and return. Used to fill in the structures.
        ...
        u32         data_version =  0xffffffff;         // Bersion read from blob or written into blob
    ...
    template<typename T>
    T* BlobSerializer::write_and_serialize( Allocator* allocator_, u32 serializer_version_, sizet size, T* data ) {

        RASSERT( data ); // Should always have data as parameter!
        ...
        // Save the root data memory for offset location
        data_memory = ( char* )data;

        // Serialize the root data
        serialize( data );
    }
    ...
    T* BlobSerializer::read( Allocator* allocator_, u32 serializer_version_, sizet size, char* blob_memory_, bool force_serialization ) {
        ...
        // If serializer and data are at the same versipon, no need to serialize.
    ...
    inline void BlobSerializer::serialize( RelativeArray<T>* data ) {
        ...
        if ( is_reading ) {
            ...
            for ( u32 i = 0; i < data->size; ++i ) {
                T* destionation = &data->get()[ i ];
                serialize( destination );

                destination = destination;      // NB! This must be wrong:
            }
            // Restore serialized
            serialized_offset = cached_serialized;
        } else {
    ...
    inline void BlobSerializer::serialize( Array<T>* data ) {
        ...
        if (is_reading) {
            ...
            // Data will be copied at the end og the current blob
            i32 data_offset = allocated_offset - serialized_offset;
